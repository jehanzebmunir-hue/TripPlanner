...
**If you create an account:** your email address and a hashed password (via bcrypt — we never store or can recover your actual password). Creating an account lets your trips show up under "My Trips" across devices, instead of relying on saving each trip's URL. If you request a password reset, a temporary token is generated and emailed to you — it's stored as a one-way hash (not the raw token itself), expires after one hour, and is deleted once used. `[ ]` *the reset email is sent through a real transactional provider (Resend), but no sending domain has been verified yet — until it is, the provider's own sandbox limitation means delivery is confirmed to work for testing, not yet guaranteed for every real account holder's inbox. See README.*
