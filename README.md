...
- **CITIES registry and language support** — flagged, not yet started. Both are large enough to need a real decision on approach before building rather than picking one silently: broadening beyond the current fixed ~159-city registry means deciding how a not-yet-registered place gets a real country/currency/timezone/market-string resolved on demand instead of from a curated table; language support means deciding real scope (UI chrome only vs. translated place data, which sources are even multi-lingual) before any i18n scaffolding goes in.
