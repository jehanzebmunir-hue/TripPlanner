...
A working slice of the trip planner concept — discovery + freshness pipeline for a solo traveler, now live in 144 pilot cities across all six inhabited continents, per the concept brief and system architecture.
...
This is the real-world version of the "not every city needs a bespoke adapter" argument — 140 of 144 cities run on the generic adapters alone and are no worse off for it. It's also a live demonstration that "checked and rejected" needs an expiry date: Boston sat in the rejected column for the entire Tier 1–5 rollout on a one-line "runs on CKAN" note, and the real dataset was there the whole time.
...
- **Google Places** — real call structure, but metered/paid; add `GOOGLE_PLACES_API_KEY` to enable. Off by default. **Now has a real, persisted spend guard**, not just a key gate: `lib/rateLimiter.ts` checks a per-day call count *before* every call and skips it once `GOOGLE_PLACES_MAX_CALLS_PER_DAY` (default 200 — a conservative placeholder, not a researched dollar figure; check current Google Places pricing and set this deliberately before ever adding a real key) is reached, rather than relying on scheduler cadence alone to keep spend bounded. Counted via a small `ApiCallBudget` table (one row per adapter per day) so the count survives a dev-server restart, and works identically whether the datastore is SQLite (now) or Postgres (later) — it's a plain upsert-and-count, no datastore-specific feature involved.
...
- **Scheduler** — now genuinely tiered rather than one flat interval: `seed` runs once at startup (it's local data, nothing upstream to go stale against), `google-places` refreshes on its own slower cadence (`STRUCTURED_REFRESH_MS`, default 6h), and `ticketmaster`/`seatgeek`/the municipal event adapters refresh hourly (`VOLATILE_REFRESH_MS`). Every city's initial run is staggered across one full refresh window rather than firing all 144 cities' worth of API calls in the same instant — see `scheduler.ts`. Each adapter outcome is now `{ count, ok, error? }` rather than a bare number, so a dead API and a genuinely quiet market no longer look identical in the logs (`ingestion.service.ts`).
...
- **Exchange-rate sync — real structure, no provider connected.** `lib/exchangeRates.ts` + `GET /api/exchange-rate?from=&to=` follow the same pattern as `lib/email.ts`: a generic `{rate: number}` contract (not any specific real provider's actual API shape, since none has been chosen or verified against — whichever one gets picked will likely need a thin adapter in front of it), a real per-pair-per-UTC-day cache (`ExchangeRateCache`) so a connected provider gets at most one live call per pair per day rather than one per request, and an honest `null` — not a guessed or hardcoded rate — when nothing is configured. Verified live: same-currency short-circuits to `1` with zero DB/network calls, and an unconfigured provider returns `{rate: null}` with an empty cache. **What this doesn't do, and can't yet regardless of any key**: there is no "traveler's home currency" anywhere in the app — no field on `User` or `Trip` stores one — so nothing calls this endpoint from the UI yet. There's no real answer to "convert to what" without that preference existing first, and that's a product decision (where does it come from — an account setting, a per-trip field, browser locale?), not a technical one to guess at.
...
- **Automated tests, backend and frontend both** — backend: `vitest`, `npm test`, 74 tests across 17 files (confidence-decay engine, ingestion incl. failure-vs-empty distinction, adapter-health recording, and priceAmount survival through upsert, Chicago festival grouping, market-collision guard checked live against all 144 real cities, travel-profile coverage, destination-recommender ranking/filtering, retry/backoff behavior, password-reset token generation/consumption, account deletion, stale-trip and reset-token retention sweeps, production-safe email logging, Google Places spend-guard behavior, real price extraction in the Ticketmaster/SeatGeek adapters, exchange-rate cache/fallback behavior). Frontend: `vitest` + React Testing Library, 18 tests across 5 files (SetupScreen's default/submit/disabled-state behavior, ItineraryScreen's timezone-correct day labels — including a regression test for the exact date-shift bug the timezone fix resolved — FindDestinationPanel's collapsed-by-default and real-vibe-list behavior, DiscoverScreen's degraded-data banner and price rendering across free/priced/unverified states, AccountPanel's forgot-password flow and two-click account-deletion confirmation). There was no test suite at all — backend or frontend — before this pass; every prior claim of "verified" or "smoke-tested" in this document came from one-off manual `curl`/ingest runs, not a repeatable check.
- **CI** — `.github/workflows/ci.yml` runs both test suites (plus typecheck, collision-check, and frontend build) on push/PR. Not live yet: this project has no git history and no GitHub remote (see below), so the workflow is ready but inert until that exists — nothing about the workflow itself needs to change once it does.
...
- **Version control** — this project still isn't a git repository. Nothing about that blocks local development, but it does mean the CI workflow above can't run, there's no commit history to review, and there's no way to push anywhere. Worth flagging plainly rather than leaving implicit.

## Adding another city (or country)

1. Add an entry to `backend/src/config/cities.ts` (slug, display name, `country` code, Ticketmaster/SeatGeek market names). That alone makes it a working city on the default adapters — verify first that Ticketmaster/SeatGeek actually have coverage there before assuming it.
...
- `backend/src/adapters/` — one file per external source, normalized to a common record shape. `index.ts` also defines `DEFAULT_ADAPTERS`.
...
- `backend/src/services/ingestion.service.ts` — runs a city's default + extra adapters (optionally filtered, for the tiered scheduler), upserts into `Place`, and records per-adapter health; returns `{ count, ok, error? }` per adapter.
- `backend/src/scheduler.ts` — per-adapter-tier refresh cadence with staggered initial runs (see "What's real vs. stubbed").
