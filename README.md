...
- **Nav-tab icons — designed via a dedicated Claude Design pass, not a default pick.** The app had zero icons anywhere before this — every control was a plain text label, a deliberate choice this document already covered the tradeoffs of. Given the go-ahead to add a small, generic set for just the 4 nav tabs, the actual visual/style decision was handed to a separate Claude Design prompt (built from this app's real, existing tokens — the paper/ink/accent palette, both light and dark, the serif/mono type system, the sharp-cornered no-`rounded-lg` restraint) rather than picked unilaterally. Landed on `lucide-react`'s `SlidersHorizontal` (Setup), `Compass` (Discover), `CalendarDays` (Itinerary), and `ListChecks` (Checklist), rendered with `strokeLinecap="square"`/`strokeLinejoin="miter"` to override Lucide's own rounded default caps/joins and match the app's sharp-corner aesthetic instead. Icons sit next to the existing text labels, never replacing them, and are `aria-hidden` (decorative, not a second accessible name). Deliberately generic/unbranded — the product's own name isn't finalized yet, so nothing icon-specific to a current name or logo (see "naming uncertain" going forward). Production bundle grew by ~1.5KB gzipped (94.15KB → 95.64KB) despite `lucide-react` exporting 1000+ icons — real ESM tree-shaking, confirmed by direct before/after build measurement, not assumed.
- **A real, reproducible test-suite flakiness found and fixed while adding the icon library, unrelated to icons themselves.** Adding `lucide-react` pushed full-suite parallel test runs into genuine, repeatable resource contention — different, unrelated tests (`SetupScreen`, `AccountPanel`) intermittently blew through their default timeouts depending on which one lost that particular run's contention, while every one of them passed quickly and cleanly in isolation every time. Verified this wasn't a real app regression before treating it as a test-infra issue: isolated runs, repeated full-suite runs, and a reduced-worker-count run all confirmed the same thing — the application code was correct and fast throughout. Fixed with a single global `testTimeout` increase in `vitest.config.ts` (5s → 15s) rather than chasing whichever individual test happened to time out on a given run.
